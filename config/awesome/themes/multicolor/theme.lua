...
-- PipeWire volume (wpctl)
local volicon = wibox.widget.imagebox(theme.widget_vol)

local volume_level = 0
local volume_muted = false

local volume_display = wibox.widget.textbox()
volume_display.font = theme.font

local function update_volume_text()
    local text = tostring(math.floor(tonumber(volume_level) or 0))
    if volume_muted then
        text = text .. "M"
    end
    volume_display:set_markup(markup.fontfg(theme.font, "#7493d2", text .. "% "))
end
...
function volume_bar:set_volume_level(new_value)
    volume_level = new_value
    update_volume_text()
end

function volume_bar:mute()
    volume_muted = true
    update_volume_text()
end

function volume_bar:unmute()
    volume_muted = false
    update_volume_text()
end

local volume_widget = require("awesome-wm-widgets.wpctl-widget.volume")
theme.volume_widget = volume_widget
theme.volume = {
    widget = volume_widget({
        custom_widget = volume_bar,
